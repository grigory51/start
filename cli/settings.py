...
sidecar-контроль; (b) убираем enabledPlugins/extraKnownMarketplaces выключенных (их
маркетплейс в seed не собран — false-ключ дал бы CC cache-miss); (c) пишем
...
def build_fragment(plugins: list[config.Plugin]) -> dict:
    ...
    """
    # Только включённые плагины попадают в enabledPlugins. Выключенные НЕ пишем как
    # false: их маркетплейс в seed не собран (build_seed ставит лишь enabled), а CC по
    # ключу enabledPlugins парсит маркетплейс и пытается его загрузить — для несобранного
    # это «Marketplace <mp> failed to load: cache-miss». Отсутствие ключа = выключен,
    # без висячей ссылки. Прошлый false подчистит sidecar-прун (stale-ключ).
    enabled_plugins: dict[str, bool] = {}
    marketplaces: dict[str, dict] = {}
    for p in plugins:
        if p.enabled:
            enabled_plugins[p.ref] = True
