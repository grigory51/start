...
- Для HUD, меню, инвентарей и строительных панелей сначала читай `game-ui-frontend` (в Codex: `game-studio:game-ui-frontend`); для Three.js — также `threejs-game-ui-designer`. Общие frontend-скилы дополняют оформление после выбора игрового взаимодействия.
- Структуру определяют действия игрока, их частота, жанр, камера и целевые устройства. Различай live HUD, инструменты режима, расширенный каталог и pause/settings. Плотный каталог допустим там, где игрок сравнивает и выбирает; компактность не оправдывает непонятные иконки и лишние действия.
- При существенной переделке опирайся на разобранные игровые референсы. Проверяй полный путь выбора, применения, отмены и возврата в игру, включая повторное использование. Перенос панели не заменяет исправление взаимодействия.
- Визуальное качество проверяется поверх реальной сцены и в движении: обзор объекта действия, читаемость, состояния элементов, фокус и границы ввода камеры/UI. Без разрешённого playtest или материалов владельца явно отмечай, что UI/UX не проверен.
