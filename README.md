...
- `manage` — Textual TUI с двумя вкладками. **Агенты**: сгруппированы по источнику, Enter открывает модалку с телом `.md`. **Скилы**: сгруппированы по источнику, Enter открывает `SKILL.md`, Space/`t` включает/выключает (toggle пишет в `config.local.toml`, версионный `config.toml` не трогает, и тут же дёргает линковку), `a` — добавить git-сабмодуль (см. ниже).
- `add-submodule <url>` — `git submodule add` в `contrib/`, автодетект подпапки со скилами и запись `[[skills]]` в `config.toml` (флаги `--name`, `--skills-subdir`, `--no-install`)

Создаёт symlink'и в `~/.claude/`:
- `~/.claude/agents/` — **реальная папка** с per-file симлинками; источники
  (`repo/agents`, сабмодули `contrib/<...>/agents`) перечислены в `config.toml` секцией `[[agents]]`.
- `~/.claude/skills/` — **реальная папка** с per-skill симлинками; источники
  (`repo/skills`, сабмодули `contrib/<...>`) перечислены в `config.toml` секцией `[[skills]]`.
...
Агенты — секция `[[agents]]` (аналогично скилам, но источник — папка с `*.md`-файлами; имя файла без `.md` = имя агента):
...
[[agents]]
path = "contrib/claude-seo/agents" # агенты из сабмодуля
include = "*"
```

Поштучного вкл/выкл у агентов нет — все найденные линкуются. Конфликт имён — первое вхождение (как у скилов).

Добавить новый набор:

```bash
uv run claude-agents add-submodule <url>   # git submodule add + автодетект + [[skills]] в config.toml
# для агентов из сабмодуля — допиши [[agents]] в config.toml вручную
make up
```

Под капотом `add-submodule` = `git submodule add <url> contrib/<name>` + автодетект подпапки со скилами (корень / `./skills`) + запись `[[skills]]`. Можно и руками: `git submodule add ...`, затем вписать `[[skills]]`/`[[agents]]` в `config.toml`, `make up`. То же доступно в TUI (`make manage`) по клавише `a`.
...
Запись (`[[skills]]` или `[[agents]]`) с тем же `path` заменяет базовую целиком; новый `path` добавляется; `enabled = false` выключает источник; `disabled` из обоих файлов объединяется.
...
- **Внешний набор** → `uv run claude-agents add-submodule <url>` (или `git submodule add <url> contrib/<name>` + `[[skills]]`/`[[agents]]` в `config.toml`), `make up` (см. [Установка](#установка)).
