...
- **[Claude Code](#домен-claude-code-claude)** (`~/.claude`) — агенты и их оркестрация, навыки, нативные плагины, hooks, `CLAUDE.md`, statusline, `settings.json`.
- **[Dotfiles](#домен-dotfiles-home)** (`$HOME`) — обычные dotfiles машины (`~/.vimrc`, …), к Claude отношения не имеющие.
...
Синхронизация разбита на два **домена**: **Claude Code** (`~/.claude`: агенты, скилы, плагины, hooks, CLAUDE.md, statusline, settings) и **Dotfiles** (`$HOME`: `[[dotfiles]]`). В выводе они разделены заголовками (`══ Claude Code ══` / `══ Dotfiles ══`); можно гонять по отдельности.

| Цель | Что |
|---|---|
| `make up` | оба домена — полная синхронизация (сабмодули + плагины seed + symlink'и + settings + dotfiles) |
| `make claude` | только домен Claude Code (`~/.claude`) |
| `make dotfiles` | только dotfiles (`$HOME`), без сабмодулей/seed/settings |
...
make claude -- --dry-run    # только Claude Code, вхолостую
make dotfiles -- --force    # только dotfiles, с бэкапом чужих файлов
```

CLI — `uv run start <команда>` (Python 3.11+). Подкоманды:
- `up` — оба домена (флаги `--dry-run`, `--force`, `--no-submodules`, `--no-seed`, `--no-settings`, `--only claude|dotfiles`)
...
Навык `ai/skills/my-principles/` — личные принципы/стиль, грузится во всех агентов через `skills:` frontmatter.
...
- `~/.claude/agents/` — **реальная папка** с per-file симлинками; источники
  (`repo/ai/agents`) перечислены в `config.toml` секцией `[[agents]]`.
- `~/.claude/skills/` — **реальная папка** с per-skill симлинками; источники
  (`repo/ai/skills`, сабмодуль `contrib/blender-skills`) перечислены в `config.toml` секцией `[[skills]]`.
- `~/.claude/hooks/*.sh` → `repo/ai/hooks/*.sh` (per-file)
...
> На практике seo/infostyle подключены как `[[plugins]]`; `[[skills]]` остались только для loose-скилов (`ai/skills/` + `contrib/blender-skills`), `[[agents]]` — только для своих (`ai/agents/`).
...
- `[[hooks]]` — авто-регистрация loose-хука на события CC (см. [Нотификации](#нотификации-когда-агент-ждёт-тебя)).
...
Регистрация событий автоматическая. Секция `[[hooks]]` в `config.toml` задаёт, какой скрипт на какие события CC повесить:

```toml
[[hooks]]
path = "ai/hooks/notify.sh"         # *.sh относительно корня репо
...
- Добавить звук → в `ai/hooks/notify.sh` допиши в `osascript` `... sound name "Glass"`.
...
Кроме конфига Claude Code, `start` раскатывает обычные dotfiles машины — файл/папку из репо симлинком в `$HOME`. Это часть «сетапа ноутбука», не имеющая отношения к Claude. Запуск домена по отдельности — `make dotfiles` (или `uv run start up --only dotfiles`; сабмодули/seed/settings при этом не трогаются). Секция `[[dotfiles]]` в `config.toml`:
...
# вписать [[dotfiles]] в config.toml
make dotfiles -- --force           # старый ~/.vimrc → ~/.vimrc.bak, на месте симлинк
```

> Прун снятых записей не делается: убрал `[[dotfiles]]` из конфига — симлинк в `$HOME` снимаешь вручную (безопасно итерировать `$HOME` менеджер не берётся).

## Кастомизация

- **Свой стиль** → правь `ai/skills/my-principles/SKILL.md` (общий для всех).
- **Тюнинг конкретного агента** → правь тело его `.md` в `ai/agents/`.
- **Новый агент** → добавь `ai/agents/<name>.md`, при необходимости впиши его в `tools: Agent(...)` архитектора, `make up`.
- **Внешний набор** → `uv run start add-submodule <url>` (или `git submodule add <url> contrib/<name>` + `[[skills]]`/`[[agents]]`/`[[plugins]]` в `config.toml`), `make up` (см. [Установка](#установка)).
