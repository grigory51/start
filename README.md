...
`settings.json` один на scope и общий с другими инструментами (caveman, OMC, notify, permissions, model…). Менеджер дописывает в него **только свои ключи** — `enabledPlugins`, `extraKnownMarketplaces`, `env.CLAUDE_CODE_PLUGIN_SEED_DIR`, `mcpServers`, `hooks` — и трекает их в sidecar-манифесте `~/.claude/.claude-agents-managed.json`. Чужие ключи не трогаются. Merge идемпотентен, перед записью кладётся бэкап `settings.json.bak`. Выпавшее (плагин/хук/MCP убрали из конфига) удаляется по sidecar. Подкоманда `settings --remove` чистит ровно свои ключи и удаляет sidecar.
